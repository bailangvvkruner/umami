...
import { getWebsiteById, getWebsitesByUserId, createWebsite, updateWebsite, deleteWebsite, resetWebsite } from './queries/website';
import { createSession, getActiveVisitors } from './queries/session';
import { saveEvent, getWebsiteStats, getPageviews, getMetrics, getEventsByWebsiteId } from './queries/event';
import { getTeamsByUserId, createTeam, getTeamById, deleteTeam, getTeamMembers } from './queries/team';
...
meApp.get('/websites', async (c) => {
    ...
    const { data, total } = await getWebsitesByUserId(db, user.id, { page, pageSize });
...
meApp.get('/teams', async (c) => {
    ...
    const { data, total } = await getTeamsByUserId(db, user.id, { page, pageSize });
...
websitesApp.get('/:websiteId/events', async (c) => {
    ...
    const { data, total } = await getEventsByWebsiteId(db, websiteId, {
        page,
        pageSize,
        startAt,
        endAt,
